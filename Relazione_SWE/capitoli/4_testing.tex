\chapter{Testing}
\label{ch:testing}

\section{Strategia di Verifica}

I test sono eseguiti su due livelli:

\begin{itemize}
    \item \textbf{Livello 1 -- Business Logic}: Service, componenti \texttt{core} (\texttt{GridCluster}, \texttt{GridMonitor}) e Strategy, testati isolando ogni dipendenza esterna con Mockito.
    \item \textbf{Livello 2 — Persistence Layer}: i DAO Postgres, testati contro un database H2 reale in modalità \texttt{MODE=PostgreSQL}, per verificare che le query SQL scritte a mano funzionino davvero e non solo che le interazioni tra oggetti siano quelle attese.
\end{itemize}

In tutto la suite conta \textbf{121 test}, tutti verdi.

Vale la pena dire anche cosa \emph{non} abbiamo testato, perché il confine è una scelta e non una dimenticanza. \textbf{Non ci sono test sui Controller né sull'interfaccia grafica.} Testare JavaFX richiede di far partire il toolkit e di simulare i click, con un costo alto e un guadagno basso: i Controller, per come li abbiamo scritti, non contengono logica di dominio: leggono l'azione dell'utente, chiamano un Service e mostrano il risultato. La logica che può davvero sbagliare i conti sta nei Service e nel dominio, ed è lì che abbiamo concentrato i test.

\subsection{Tecnologie e Strumenti}

\begin{itemize}
    \item \textbf{JUnit 5}: framework di esecuzione, con le annotazioni \texttt{@BeforeEach}/\texttt{@AfterEach} per il setup e cleanup di ogni test e \texttt{@BeforeAll}/\texttt{@AfterAll} per le risorse costose (come la connessione H2) condivise da un'intera classe.
    \item \textbf{Mockito}: usato non solo per mockare interfacce (\texttt{StationDao}, \texttt{SessionService}, \dots) ma anche per mockare \textbf{metodi statici} tramite \texttt{Mockito.mockStatic()} — una necessità specifica di questo progetto, perché sia \texttt{DatabaseManager.getConnection()} sia \texttt{GridCluster.getInstance()} sono chiamate statiche che altrimenti aprirebbero una connessione reale o toccherebbero lo stato globale del Singleton durante un test unitario.
    \item \textbf{H2 in modalità \texttt{MODE=PostgreSQL}}: usato esclusivamente nei test dei DAO, con lo schema reale caricato all'avvio tramite \texttt{INIT=RUNSCRIPT FROM './src/main/resources/scheme.sql'} — lo stesso file \texttt{scheme.sql} usato in produzione, quindi i test verificano le query contro la struttura di tabelle reale, non una sua approssimazione.
\end{itemize}

\section{Livello 1: Business Logic}

\subsection{Logica pura, senza mock — \texttt{ChargingStrategiesTest}}

Le strategie di ricarica (\texttt{FastChargingStrategy}, \texttt{EcoChargingStrategy}) hanno stesso input, stesso output e nessuna dipendenza esterna. Per questo motivo il loro test non usa Mockito: costruisce oggetti di dominio reali con \texttt{reconstitute()} e confronta il risultato con un secondo calcolo indipendente, scritto direttamente nel test:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
private double basePrice(double kwh, SubscriptionPlan plan) {
    double discount = (plan != null) ? plan.getDiscount() : 0.0;
    BigDecimal platAfter = TARIFF_PLAT.multiply(BigDecimal.ONE.subtract(BigDecimal.valueOf(discount)));
    BigDecimal rate = TARIFF_OP.add(platAfter);
    return rate.multiply(BigDecimal.valueOf(kwh)).setScale(2, RoundingMode.HALF_UP).doubleValue();
}

@Test
void testEcoCalculateCostWithSubscriptionDiscount() {
    double kwh = 100.0;
    Driver premiumDriver = testDriver(3L, "Anna", "anna@test.com", SubscriptionPlan.PREMIUM);
    double expectedPremium = round2(basePrice(kwh, SubscriptionPlan.PREMIUM) * 0.90);
    assertEquals(expectedPremium, ecoStrategy.calculateCost(kwh, mockStation, premiumDriver), 0.001,
            "PREMIUM: sconto piano sulla sola quota piattaforma, poi Eco 10%");
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Riscrivere la formula nel test invece di limitarsi a un valore atteso hardcoded (es. \texttt{assertEquals(33.30, ...)}) ha un vantaggio: se le tariffe di test cambiano, il valore atteso si ricalcola automaticamente insieme a esse, e il test continua a verificare la \emph{relazione} tra i numeri (lo sconto si applica solo alla quota piattaforma) invece di un singolo numero fisso che potrebbe diventare scorrelato dalla logica reale.

Va però riconosciuto il rovescio della medaglia, perché è un'obiezione legittima: duplicando la formula di produzione dentro l'oracolo del test, un errore \emph{sistematico} nella formula verrebbe copiato in entrambe le copie e il test passerebbe lo stesso. Un oracolo scritto ricalcolando la stessa logica non può, da solo, dimostrarne la correttezza. La scelta resta comunque difendibile, a patto di essere chiari su cosa questi test verificano davvero: non il valore assoluto del prezzo, ma l'\textbf{invariante di dominio} che quel valore deve rispettare, cioè che lo sconto dell'abbonamento morda \emph{solo} sulla quota della piattaforma e mai su quella dell'operatore. Non a caso l'oracolo non ricopia \texttt{priceFor()} riga per riga: ricostruisce il prezzo a partire dalle due componenti separate, e sarebbe quindi il primo a cadere se qualcuno, in produzione, applicasse per errore lo sconto anche alla quota dell'operatore.

Questa regola merita una precisazione, perché nel codice convivono due sconti diversi. Lo sconto \emph{dell'abbonamento} si applica alla sola quota piattaforma; lo sconto \emph{della modalità Eco} (un ulteriore 10\%) si applica invece al prezzo base complessivo. Non è un'incoerenza: in entrambi i casi lo sconto è a carico della piattaforma, perché allo Station Operator viene comunque accreditata la sua quota piena sull'energia erogata (paragrafo~\ref{subsec:sessionservice}). È la piattaforma a finanziare le promozioni, non chi possiede la colonnina.

\subsection{Singleton e reset dello stato : \texttt{GridClusterTest}}

\texttt{GridCluster} è un Singleton (Capitolo~\ref{ch:pattern} -- Idiomi e Design Pattern): la sua istanza sopravvive tra un test e l'altro se non viene esplicitamente distrutta, col rischio che i dati caricati da un test "contaminino" il test successivo. La soluzione adottata è azzerare il campo statico \texttt{instance} via reflection prima di ogni test:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
@BeforeEach
void setUp() throws Exception {
    Field instanceField = GridCluster.class.getDeclaredField("instance");
    instanceField.setAccessible(true);
    instanceField.set(null, null);

    gridCluster = GridCluster.getInstance();
    transformerDaoMock = Mockito.mock(TransformerDao.class);
    stationDaoMock = Mockito.mock(StationDao.class);
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Con l'istanza garantita pulita, il test su \texttt{getNearestAvailable()} verifica sia la logica di filtro (solo le colonnine \texttt{ACTIVE}) sia quella di ordinamento (per distanza crescente), usando dati costruiti ad hoc invece di un vero calcolo GPS:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
List<ChargingStation> nearest = gridCluster.getNearestAvailable(0.0, 0.0);

assertEquals(2, nearest.size(), "Deve escludere le stazioni non ACTIVE");
assertEquals("Vicino", nearest.get(0).getName(), "La prima deve essere quella geometricamente piu vicina");
assertEquals("Lontano", nearest.get(1).getName(), "La seconda deve essere quella piu lontana");
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

\subsection{La tecnica: intercettare la transazione — \texttt{DriverServiceTest}}

I Service che scrivono sul database aprono e chiudono la transazione da soli (\texttt{setAutoCommit(false)}, poi \texttt{commit()} oppure \texttt{rollback()} se qualcosa va storto, e in ogni caso \texttt{close()}). Per testare questo comportamento dobbiamo mettere le mani sulla connessione prima che il Service la apra davvero: da qui il mock statico.

Usiamo come esempio \texttt{DriverServiceTest}, cioè il servizio più semplice che abbiamo: sposta la posizione di un Driver e la salva, nient'altro. Proprio perché non fa altro, mostra la tecnica senza rumore attorno. I test che contano davvero sono quelli dei due paragrafi successivi, e usano tutti questo stesso schema.

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption=Mock statico di DatabaseManager per intercettare la connessione JDBC.]
@BeforeEach
void setUp() throws SQLException {
    connectionMock = mock(Connection.class);
    mockedDbManager = mockStatic(DatabaseManager.class);
    mockedDbManager.when(DatabaseManager::getConnection).thenReturn(connectionMock);

    daoFactoryMock = mock(DaoFactory.class);
    userDaoMock = mock(UserDao.class);
    when(daoFactoryMock.createUserDao(connectionMock)).thenReturn(userDaoMock);

    driverService = new DriverService(daoFactoryMock);
}

@AfterEach
void tearDown() {
    if (mockedDbManager != null) {
        mockedDbManager.close();  // fondamentale: rilascia il mock statico globale
    }
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Con la connessione sotto controllo, si possono verificare separatamente il percorso di successo e quello di fallimento — due test speculari che verificano esattamente le proprietà opposte sulla stessa transazione:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
@Test
void testUpdateDriverLocation_Success() throws SQLException {
    Driver mockDriver = mock(Driver.class);
    driverService.updateDriverLocation(mockDriver, 43.7696, 11.2558);

    verify(mockDriver, times(1)).updatePosition(43.7696, 11.2558);
    verify(userDaoMock, times(1)).update(mockDriver);
    verify(connectionMock, times(1)).commit();
    verify(connectionMock, times(1)).close();
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
@Test
void testUpdateDriverLocation_Failure() throws SQLException {
    doThrow(new RuntimeException("Database error")).when(userDaoMock).update(any(Driver.class));

    assertThrows(RuntimeException.class, () ->
        driverService.updateDriverLocation(mockDriver, 0.0, 0.0));

    verify(connectionMock, times(1)).rollback();
    verify(connectionMock, times(1)).close();
    verify(connectionMock, never()).commit();
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

La coppia di test non verifica solo che "il metodo non esplode": verifica che, se qualcosa va storto, il sistema non lasci una transazione aperta a metà. Un test che guardasse solo il valore di ritorno non se ne accorgerebbe.

\subsection{Il cuore economico: \texttt{SessionServiceTest}}

Il \texttt{SessionService} è il servizio più esposto del sistema, perché viene invocato ad ogni tick per ogni ricarica in corso ed è l'unico che muove denaro. È quindi anche il più testato (19 test). Ne riportiamo due, scelti perché verificano due proprietà che una semplice ispezione del codice non garantirebbe.

Il primo riguarda \textbf{dove} vengono fatti i controlli di business. Un Driver senza fondi non deve poter aprire una sessione: la regola vive nel dominio (\texttt{ChargingSession.open()}), e il test verifica non solo che l'eccezione venga lanciata, ma che ciò accada \emph{prima} di toccare il database:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption={La validazione di dominio precede l'accesso al database}]
InsufficientBalanceException ex = assertThrows(InsufficientBalanceException.class, () ->
        sessionService.openSession(poorDriver, station, strategy, 20.0)
);
assertTrue(ex.getMessage().contains("insufficiente"));

// Il fallimento avviene PRIMA di toccare il DB
verify(daoFactoryMock, never()).createSessionDao(any());
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

L'ultima riga è la parte interessante: \texttt{verify(..., never())} sulla factory dei DAO non verifica un risultato, ma un \emph{non-evento}. Fissa per contratto che una sessione senza copertura economica non arrivi nemmeno ad aprire una connessione: un test black-box sul solo valore di ritorno non potrebbe distinguere questo caso da uno in cui il database viene interrogato e poi il lavoro annullato.

Il secondo test verifica l'invariante contabile più importante del sistema, cioè che \textbf{allo Station Operator venga accreditata la sua quota, e solo la sua}. La quota è definita come tariffa operatore $\times$ kWh erogati; sul valore effettivamente accreditato si usa un \texttt{ArgumentCaptor}:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption={Cattura dell'importo accreditato all'operatore alla chiusura}]
ChargingSession session = ChargingSession.open(driver, station, ChargingType.FAST, 20.0);
session.addTick(10.0, new BigDecimal("5.00"));   // kWh erogati = 10.0

sessionService.closeSession(session);

ArgumentCaptor<BigDecimal> captor = ArgumentCaptor.forClass(BigDecimal.class);
verify(fullOperator, times(1)).addEarnings(captor.capture());
assertEquals(0, captor.getValue().compareTo(new BigDecimal("3.00")),
        "La quota deve essere 0.30 x 10 = 3.00");
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Due dettagli meritano attenzione. Il primo è il confronto: si usa \texttt{compareTo(...) == 0} e non \texttt{assertEquals} diretto sui \texttt{BigDecimal}, perché \texttt{BigDecimal.equals()} confronta anche la \emph{scala}, e considererebbe \texttt{3.00} diverso da \texttt{3.0}. Su un test che verifica denaro, un confronto ingenuo produrrebbe falsi allarmi: si confrontano i \emph{valori}, non la loro rappresentazione. Il secondo è la scelta di \texttt{times(1)}: insieme ai test gemelli \texttt{testCloseSession\_NoOperator\_NoCredit} e \texttt{testForceClose\_CreditsOperator}, questo verifica la guardia \texttt{wasActive} discussa nel paragrafo~\ref{subsec:sessionservice}, cioè che l'operatore non possa essere pagato due volte per la stessa energia.

\subsection{La concorrenza sulle prenotazioni: \texttt{StationServiceTest}}

L'altro punto delicato è l'acquisizione di una prenotazione, dove il servizio si affida a un update condizionale del database anziché a una coppia lettura/scrittura. Il test simula lo scenario di corsa mockando la risposta del DAO: \texttt{acquireAtomicHold} restituisce \texttt{false}, cioè "la \texttt{UPDATE} non ha aggiornato alcuna riga, qualcun altro è arrivato prima".

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption={Il perdente della corsa non prenota, e non lascia una transazione a metà}]
// Simuliamo che qualcun altro abbia gia' preso la colonnina
when(stationDaoMock.acquireAtomicHold(100L, 1L)).thenReturn(false);

StationNotAvailableException exception = assertThrows(StationNotAvailableException.class, () ->
        stationService.hold(mockDriver, mockStation));

verify(mockStation, never()).setReserved(any());   // niente mutazione in memoria
verify(connectionMock, never()).commit();          // niente commit
verify(connectionMock, times(1)).rollback();       // e la transazione viene chiusa pulita
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Le tre \texttt{verify} finali sono il vero contenuto del test, e verificano tre proprietà distinte: che l'oggetto in memoria non venga sporcato con una prenotazione che il database ha rifiutato (altrimenti memoria e DB divergerebbero), che nulla venga confermato, e che la transazione venga esplicitamente annullata invece di essere semplicemente abbandonata. Il test speculare (\texttt{testHold\_Success}) verifica il percorso opposto sulla stessa transazione.

\subsection{L'emergenza termica: \texttt{LoadBalancerServiceTest}}

Qui i mock statici diventano due: oltre a \texttt{DatabaseManager} va sostituito anche il Singleton \texttt{GridCluster}, altrimenti il test andrebbe a leggere la cache vera della rete.

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption=Doppio mock statico: connessione JDBC e Singleton GridCluster.]
mockedDbManagerStatic = mockStatic(DatabaseManager.class);
mockedDbManagerStatic.when(DatabaseManager::getConnection).thenReturn(connectionMock);

gridClusterMock = mock(GridCluster.class);
mockedGridClusterStatic = mockStatic(GridCluster.class);
mockedGridClusterStatic.when(GridCluster::getInstance).thenReturn(gridClusterMock);
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Il test lancia l'evento \texttt{THERMAL\_ALERT} e controlla tre cose in un colpo solo: che la colonnina venga messa in sovraccarico, che la modifica finisca davvero nel database, e che la chiusura della sessione venga \emph{delegata} al \texttt{SessionService}.

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
loadBalancerService.update(transformerMock, TransformerEvent.THERMAL_ALERT);

verify(stationMock, times(1)).setOverloaded();
verify(stationDaoMock, times(1)).update(stationMock);
verify(connectionMock, times(1)).commit();
verify(sessionServiceMock, times(1)).forceClose(sessionMock);
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

L'ultima riga è quella che conta: il \texttt{LoadBalancerService} non sa \emph{come} si chiude una sessione (rimborso, wallet, ricevuta), sa solo \emph{che} va chiusa. È il punto dell'Observer descritto nel Capitolo~\ref{ch:pattern}, verificato da un test invece che affermato a parole.

\section{Livello 2: Persistence Layer (DAO)}

I test dei DAO non usano mock: interrogano un database vero, creato all'avvio della classe di test con lo stesso \texttt{scheme.sql} della produzione. Le query vengono quindi eseguite davvero, contro le tabelle e i vincoli reali, non contro un'imitazione.

C'è però un limite che va detto subito, perché è la prima cosa che salta all'occhio: in produzione i DAO parlano con \textbf{PostgreSQL}, mentre questi test girano su \textbf{H2}. Anche in modalità \texttt{MODE=PostgreSQL}, H2 resta un motore diverso: la sintassi che accetta è quasi la stessa, ma non è garantito che lo siano tutti i comportamenti ai bordi (conversioni di tipo, ordinamenti, messaggi d'errore). Un test verde su H2 non è quindi la prova assoluta che la stessa query passi su Postgres.

È un compromesso preso in modo consapevole: H2 gira in memoria, parte in un istante e non richiede di avere un server Postgres acceso per lanciare i test. In cambio si rinuncia a testare il motore di produzione. Ci siamo tenuti stretti su una cosa, però: lo schema caricato è lo \emph{stesso file} usato in produzione, quindi se una query rompesse un vincolo o sbagliasse un nome di colonna, il test se ne accorgerebbe comunque.

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption=Avvio di H2 con lo schema reale di produzione.]
@BeforeAll
static void startDatabase() throws SQLException {
    String url = "jdbc:h2:mem:chargenet_test;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;"
               + "DEFAULT_NULL_ORDERING=HIGH;INIT=RUNSCRIPT FROM './src/main/resources/scheme.sql'";
    connection = DriverManager.getConnection(url, "sa", "");
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Poiché la connessione è condivisa da tutta la classe (\texttt{@BeforeAll}, non \texttt{@BeforeEach}, per non pagare il costo di riavviare H2 ad ogni singolo test), ogni test deve invece garantire di ripartire da una tabella vuota. Questo pone un problema: la tabella \texttt{charging\_sessions} ha una foreign key verso \texttt{charging\_stations}, quindi uno svuotamento diretto verrebbe rifiutato dal database finché non si disattivano temporaneamente i vincoli:

\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption=Pulizia delle tabelle rispettando temporaneamente i vincoli di integrità.]
try (Statement stmt = connection.createStatement()) {
    stmt.execute("SET REFERENTIAL_INTEGRITY FALSE");

    stmt.execute("TRUNCATE TABLE charging_sessions RESTART IDENTITY");
    stmt.execute("TRUNCATE TABLE charging_stations RESTART IDENTITY");
    stmt.execute("TRUNCATE TABLE users RESTART IDENTITY");
    stmt.execute("TRUNCATE TABLE power_transformers RESTART IDENTITY");

    // ... reinserimento dei dati minimi necessari (operatore, driver, trasformatore, stazione) ...

    stmt.execute("SET REFERENTIAL_INTEGRITY TRUE");
}
\end{lstlisting}
\end{minipage}
\vspace{0.3cm}

Il vantaggio di questo approccio rispetto a un semplice \texttt{DELETE FROM}: \texttt{RESTART IDENTITY} azzera anche i contatori auto-incrementali, così ogni test parte con ID prevedibili