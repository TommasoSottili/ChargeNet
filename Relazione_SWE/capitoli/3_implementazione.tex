...
Mentre il Load Balancer gestisce le emergenze, il \texttt{SessionService} coordina la normale operatività economica ed energetica. La parte delicata in questa classe è stata garantire una corretta gestione unica di due scritture correlate (sessione e portafoglio) all'interno della stessa transazione

Ogni volta che una ricarica avanza (tramite il metodo \texttt{addTick()}) o viene interrotta, il sistema deve aggiornare contemporaneamente la sessione e il portafoglio del Driver. La colonnina invece cambia stato solo all'apertura e alla chiusura, non ad ogni tick.
...
\vspace{0.3cm}
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java, caption={Test di validazione di una stazione}]
