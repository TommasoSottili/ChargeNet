...
Questo capitolo descrive le scelte implementative e l'architettura interna di ChargeNet. Per mantenere la trattazione chiara, non verrà esposto l'intero codice sorgente, ma ci si concentrerà sull'organizzazione dei componenti principali e sui frammenti di codice che illustrano le logiche più complesse e i pattern utilizzati.

\section{Architettura del Sistema}
L'applicativo è stato diviso in pacchetti logici distinti per garantire un elevato disaccoppiamento e facilitare la manutenzione. L'interazione con il database PostgreSQL è gestita tramite il pattern Data Access Object (DAO), che nasconde la complessità delle query SQL al resto dell'applicazione. I livelli principali sono tre: Domain, Business e Controller.

\section{Domain Model}
Il Domain Model rappresenta il nucleo concettuale del sistema. Prima di scendere nei dettagli implementativi dei singoli moduli, è utile fornire un quadro d'insieme delle classi principali che compongono questo strato.

L'intera architettura ruota attorno a tre macro-aree strettamente interconnesse:
\begin{itemize}
    \item \textbf{Gli Attori:} Modellati dalla classe base \textit{User}, da cui derivano i profili specifici \textit{Driver}, \textit{StationOperator} ed \textit{EnergyManager}.
    \item \textbf{L'Infrastruttura:} Rappresentata logicamente dalle \textit{ChargingStation} (i punti di ricarica) e dai \textit{PowerTransformer} (i nodi di smistamento della rete elettrica).
    \item \textbf{Le Operazioni:} Gestite principalmente dalla \textit{ChargingSession}, che controlla l'erogazione di energia, supportata da entità secondarie per i pagamenti (\textit{Wallet}) e le recensioni (\textit{Feedback}).
\end{itemize}

Per mantenere il codice ordinato e coeso, queste classi sono state organizzate in cinque sotto-pacchetti. Di seguito vengono analizzate le scelte implementative più rilevanti per ciascun modulo.
...
public static ChargingStation register(StationOperator operator, PowerTransformer transformer, 
                                       Double lat, Double lng, ConnectorType connectorType, 
                                       Double powerKw, BigDecimal tariff) {
    // [...] Ometto i controlli geografici per brevita'
...
\noindent\begin{minipage}{\linewidth}
\begin{lstlisting}[language=Java]
public static ChargingSession open(Driver driver, ChargingStation station, 
                                   ChargingType strategy, Double batteryStart) {
